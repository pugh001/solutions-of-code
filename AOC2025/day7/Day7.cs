...
public class Day7
{
  ...
  {
    var data = SetupInputFile.OpenFile(input).ToArray();
    var grid = new Grid(data);
    
    var startPos = grid.FindFirst('S') ?? Point2D<int>.Origin;
    
    var part1 = CountSplits(grid, startPos);
    var part2 = CountUniquePaths(grid, startPos);
  ...
  private long CountUniquePaths(Grid grid, Point2D<int> startPos)
  {
    ...
    var dp = new long[grid.Rows, grid.Cols];
    ...
    for (int y = startPos.Y; y < grid.Rows - 1; y++)
    {
      for (int x = 0; x < grid.Cols; x++)
      {
        ...
        var currentPaths = dp[y, x];
        var downY = y + 1;
        
        // Check what's below this position
        if (downY < grid.Rows)
        {
          var belowChar = grid[downY, x];
