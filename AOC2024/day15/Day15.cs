...
public class Day15
{
  private readonly List<List<Coordinate2D>> _boxesToMove = [];
  private readonly List<Coordinate2D> _movedPositions = [];
  private readonly Queue<char> _moves = new();
  ...
  public (string, string) Process(string input)
  {
    ...
    string scaledGrid = sections[0].Replace("#", "##").Replace("O", "[]").Replace(".", "..").Replace("@", "@.");
    ...
    char[] instructions = _moveString.Where(c => "^v<>".Contains(c)).ToArray();
  ...
  private long ProcessPart2()
  {
    ...
    while (_moves.TryDequeue(out char move))
    {
      ...
      char nextCell = _map2.GetValueOrDefault(nextPos, '#');
    ...
    return _map2.Where(kvp => kvp.Value == '[').Sum(kvp => 100 * kvp.Key.Y + kvp.Key.X);
  }
  ...
  private void ExecuteBoxMoves(CompassDirection direction)
  {
    ...
    var emptyPositions = _boxesToMove.SelectMany(box => box).Where(pos => !_movedPositions.Contains(pos));
  ...
  private bool TryMoveBox(CompassDirection direction, Coordinate2D current, Coordinate2D next, bool partTwo = false)
  {
    ...
    char nextCell = _map2.GetValueOrDefault(next, '#');
  ...
  {
    return map.Where(kvp => kvp.Value is 'O' or '[').Sum(kvp => kvp.Key.Y * 100 + kvp.Key.X);
  }
