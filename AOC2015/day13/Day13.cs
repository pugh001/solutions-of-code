...
public class Day13
{
  public (string, string) Process(string input)
  {
    ...
    long part1 = CalculateMaxHappiness(people, happiness);

    // Add myself with neutral happiness
    AddNeutralPerson("me", people, happiness);
    long part2 = CalculateMaxHappiness(people, happiness);
  ...
  private static Dictionary<(string, string), int> ParseHappinessData(IEnumerable<string> data)
  {
    ...
    {
      (string person1, string person2, int value) = ParseHappinessLine(line);
  ...
  private static (string person1, string person2, int happiness) ParseHappinessLine(string line)
  {
    ...
    int multiplier = gainOrLose[0] == "lose" ?
      -1 :
      1;
  ...
  {
    return Algorithms.GetPermutations(people).Select(arrangement => CalculateArrangementHappiness(arrangement, happiness)).Max();
  }
