...
public static class PolygonUtility
{
    ...
    /// </summary>
    public static bool IsPointInOrOnRectilinearPolygon<T>(Point2D<T> point, IList<Point2D<T>> corners) 
        where T : INumber<T>
    {
    ...
    /// </summary>
    public static bool IsPointOnPolygonBoundary<T>(Point2D<T> point, IList<Point2D<T>> corners) 
        where T : INumber<T>
    {
    ...
    /// </summary>
    public static bool IsPointOnRectilinearEdge<T>(Point2D<T> point, Point2D<T> start, Point2D<T> end) 
        where T : INumber<T>
    {
    ...
    /// </summary>
    public static bool IsPointInsidePolygon<T>(Point2D<T> point, IList<Point2D<T>> corners) 
        where T : INumber<T>
    {
        int intersections = 0;
        ...
        where T : INumber<T>
    {
        ...
        {
            var x = start.X;
            var edgeMinY = start.Y < end.Y ? start.Y : end.Y;
            var edgeMaxY = start.Y > end.Y ? start.Y : end.Y;

            // Check if vertical edge cuts through rectangle
            return x > rectangle.MinX && x < rectangle.MaxX && 
                   edgeMinY < rectangle.MaxY && edgeMaxY > rectangle.MinY;
        }
        ...
        {
            var y = start.Y;
            var edgeMinX = start.X < end.X ? start.X : end.X;
            var edgeMaxX = start.X > end.X ? start.X : end.X;

            // Check if horizontal edge cuts through rectangle
            return y > rectangle.MinY && y < rectangle.MaxY && 
                   edgeMinX < rectangle.MaxX && edgeMaxX > rectangle.MinX;
        }
    ...
    /// </summary>
    public static T FindLargestRectangleArea<T>(IList<Point2D<T>> polygonCorners) 
        where T : INumber<T>
    {
        var maxArea = T.Zero;
